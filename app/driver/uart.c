...
uart0_rx_intr_handler(void *para)
{
    ...
    while (READ_PERI_REG(UART_STATUS(UART0)) & (UART_RXFIFO_CNT << UART_RXFIFO_CNT_S)) {
		...
		os_printf("RcvChar=0x%2x\n", RcvChar);
        /* you can add your handle code below.*/
		if(uart_msg.state == DATA_IDLE)
		{
	        //*(pRxBuff->pWritePos) = RcvChar;
			uart_msg.msg_buf[uart_msg.pos++] = RcvChar;
			//pRxBuff->pWritePos++;
			//os_printf("pos=0x%2x, RcvChar=0x%2x\n", uart_msg.pos, RcvChar);

			if(uart_msg.pos == CMD_HEAD_BYTES)
			{
				uart_msg.data_len = *((uint8 *)uart_msg.msg_buf+4);
			}

			if(uart_msg.pos == (CMD_HEAD_BYTES + uart_msg.data_len)) // Complete data reception
			{
				if(*(uint8 *)uart_msg.msg_buf == 0xbd || *(uint8 *)uart_msg.msg_buf == 0xbe)
				{
